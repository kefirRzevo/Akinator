...
void akinator(char* textfile_path)
{
    ...
    while(alloc_stk.size > 0)
    {
        void* temp_ptr = nullptr;
        StackPop(&alloc_stk, &temp_ptr);
        free(temp_ptr);
    }
}
...
static void akinator_guess(tree* p_tree)
{
    ...
    while(true)
    {
        ...
        again1:
            ...
            {
                printf("Для особо одаренных ответ это или да(%s), или нет(%s), третьего не дано\n", yes, no);
                goto again1;
            }
        }
        else
        ...
        again2:
            ...
            else if(!strcmp(no, yes_no_string))
            {
                ...
                StackPush(&alloc_stk, user_answer_buf);
                StackPush(&alloc_stk, user_question_buf);
...
static void akinator_define(tree* p_tree)
{
    ...
    node* found = find_path(p_tree->root, &def_stk, user_def);
    ...
    else
    {
        ...
        {
            printf("Улыбнись, потому что фронталка заполнила такого долбаеба, который вопрос блядь %s от определителя не отличает\n", user_def);
        }
        else
        {
            ...
            void* temp_ptr = nullptr;
            StackPop(&def_stk, &temp_ptr);
            while(def_stk.size > 0)
            {
                void* temp_ptr_next = nullptr;
                StackPop(&def_stk, &temp_ptr_next);
...
input2:
    ...
    {
        printf("Ах ты шутник, закинул одинаковых челиков и смеётся. На член свой посмотри, клоун хренов.\n");
        return;
    }

    stack def1_stk = {}, def2_stk = {};
    StackCtor(&def1_stk);
    StackCtor(&def2_stk);

    node* found1 = find_path(p_tree->root, &def1_stk, first_elem);
    node* found2 = find_path(p_tree->root, &def2_stk, second_elem);
    ...
    if(found1->left != nullptr || found1->right != nullptr)
    {
        printf("Щай бы вопрос %s(1) сравнивать\n", first_elem);
        goto input1;
    }
    if(found2->left != nullptr || found2->right != nullptr)
    {
        printf("Щай бы вопрос %s(2) сравнивать\n", second_elem);
        goto input2;
    }

    void* temp_ptr1 = nullptr;
    void* temp_ptr2 = nullptr;
    void* temp_ptr1_next = nullptr;
    void* temp_ptr2_next = nullptr;

    StackPop(&def1_stk, &temp_ptr1);
    StackPop(&def2_stk, &temp_ptr2);
    StackPop(&def1_stk, &temp_ptr1_next);
    StackPop(&def2_stk, &temp_ptr2_next);
    ...
    if(temp_ptr1 == temp_ptr2 && temp_ptr1_next != temp_ptr2_next)
    {
        if(temp_ptr1_next == ((node*)temp_ptr1)->left )
        {
            printf("%s, в отличие от %s, %s", first_elem, second_elem, ((node* )temp_ptr1)->data);
        }
        else
        {
            printf("%s, в отличие от %s, %s", second_elem, first_elem, ((node* )temp_ptr2)->data);
        }
    }
    printf("\n");
...
static void akinator_save(tree* p_tree)
{
    ...
    sprintf(datafile_path, "%s%zu.txt", datafile_begin, N_SAVINGS);
