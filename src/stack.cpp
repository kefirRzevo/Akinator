...
static uint64_t verify_stack(stack *const p_stack)
{
    ...
    if (p_stack->capacity < 0 || p_stack->capacity > MAX_CAPACITY) 
    {
...
{  
    ...
    if(current_cmd == push && ( (error & INVALID_DATA) == 0))                           
        fprintf(LOGFILE, "%-12" SPECIFICATOR "|", (char* )element);                             
    else if(current_cmd == pop && ( (error & INVALID_EMPTY_STACK) == 0))                
        fprintf(LOGFILE, "%-12" SPECIFICATOR "|", *((char** )p_element));                          
    else                                                                                
        fprintf(LOGFILE, " -          |");                                              
...
void stack_dump(stack *const p_stack, const char *const stack_name)
{
    ...
    fprintf (LOGFILE,
        ...
        #ifdef StackHashProtect
                "| Hash:               %-30" HASH_SPECIFICATOR "|\n"
        #endif

        #ifdef StackCanaryProtect
                "| Left_canary:        %-30" CANARY_SPECIFICATOR "|\n"
                "| Right_canary:       %-30" CANARY_SPECIFICATOR "|\n"
        #endif
,
        stack_name,
        p_stack,                 
...
,       p_stack->LeftCanary,     
    ...
    #ifdef StackCanaryProtect
    if(data != nullptr)
        fprintf(LOGFILE, "| Data_left_canary:   %-30" CANARY_SPECIFICATOR "|\n"
                         "| Data_right_canary:  %-30" CANARY_SPECIFICATOR "|\n", DataLeftCanary, DataRightCanary);
    #endif
