...
static const char*     DOT_PATH = "logfiles/tree";
...
static void print_nodes(node* tree_node, FILE* const dotfile)
{
    ...
    {
        fprintf(dotfile,    "\t\tNode%p -> Node%p[label = \"да\", color = \"black\", fontname = \"Arial\"]\n\n", tree_node, tree_node->left);
        print_nodes(tree_node->left,  dotfile);
    }
    if(tree_node->right != nullptr)
    {
        fprintf(dotfile,    "\t\tNode%p -> Node%p[label = \"нет\", color = \"black\", fontname = \"Arial\"]\n\n", tree_node, tree_node->right);
